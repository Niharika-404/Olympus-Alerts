...
const AnomalyAlerts = ({ alertData, selectedZone, alertModelData }) => {
  ...
  useEffect(() => {
    const generateTableData = () => {
      ...
      const AnomalyAlerts = filteredAlerts.filter(alert => {
        const matchedModelAlert = alertModelData.find(modelAlert => modelAlert.AlertID === alert.AlertID);
        return matchedModelAlert && matchedModelAlert.Category === 'Anomaly';
      });
      // Map filtered alerts to table rows
      const tableRows = AnomalyAlerts.map(alert => ({
  ...
  return (
    <div>
      <div id='info' className='tooltip'>
        ...
        <span className="tooltiptext">Anomaly alerts, encompassing false positives, outliers, and noise, are classified based on their anomalous nature.</span>
      </div>
